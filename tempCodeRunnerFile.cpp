...
int main(int argc,char *argv[])
{
    ...
    string filename;
    ifstream file;                                                                            //Set filename
    cout<<"Enter Filename:\t";
    cin>>filename;
    file.open(filename.c_str());                                              //set input stream
    if(!file.good())                                                                          //Check if input file exists
    {
        perror("Error:\t");
        exit(-1);
    }
    ...
    char ch;
    while(file.get(ch))                                                                       //Read file and increase count
        Count[static_cast<unsigned char>(ch)]++;
    file.clear();                                                                             //Reset File pointers
    file.seekg(0);                                                                            //Reset File pointers
    ...
    Write_compressed(file,output_file,Huffman_codemap);                                       //Write to file
